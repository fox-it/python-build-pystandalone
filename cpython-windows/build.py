...
def exec_and_log(args, cwd, env, exit_on_failure=True):
    ...
    if p.returncode and exit_on_failure:
...
def hack_project_files(
    ...
    # PYSTANDALONE: profile files will be created for the executable, not the DLL
    for prop in (b"RequirePGCFiles", b"RequireProfileData"):
        try:
            static_replace_in_file(
                pythoncore_proj,
                b"<%s>true</%s>" % (prop, prop),
                b"<%s>false</%s>" % (prop, prop),
            )
        except NoSearchStringError:
            pass
...
def run_msbuild(
    ...
    exec_and_log(args, str(pcbuild_path), os.environ, exit_on_failure=exit_on_failure)
...
def build_cpython(
    ...
    with tempfile.TemporaryDirectory(prefix="python-build-", **tempdir_opts) as td:
        ...
        if pgo:
            ...
            if meets_python_maximum_version(python_version, "3.10"):
                run_msbuild(
                    msbuild,
                    pcbuild_path,
                    target="Build",
            ...
            for test in sorted(tests):
                ...
                # PYSTANDALONE: allow tests to fail since some of them are testing stuff we explicitly disable
                exec_and_log(
                    [
        ...
        # PYSTANDALONE: add a placeholder file to the include directory
        with (install_dir / "include" / ".empty").open("ab"):
            pass
